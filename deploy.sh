# 打包 liuli_schedule
docker build --no-cache=true -t liuliio/schedule:v0.1.3 -f schdule.Dockerfile .
# 打包使用缓存
docker build --no-cache=false -t liuliio/schedule:v0.1.3 -f schdule.Dockerfile .
# 运行
docker run -d -it --restart=always -v $PWD/pro.env:/data/code/pro.env --name liuli_schedule liuliio/schedule:v0.1.3
# 上传
docker push liuliio/schedule:v0.1.3
