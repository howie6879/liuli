...
```shell
# 注意自己的 ip
mongo --host "192.168.0.2" -u "liuli" -p "liuli"
...
```shell
mkdir -p /data/docker_data/portainer_data
docker run -d -p 8001:8000 -p 9001:9000 --name=portainer --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v /data/docker_data/portainer_data:/data portainer/portainer-ce
```
...
# 建立配置目录
mkdir -p liuli/liuli_config
tourch liuli/pro.env
tourch liuli/liuli_config/wechat.json
```
...
配置好后，请将以上内容写到`vim liuli/pro.env`文件。

接下来直接配置公众号相关，直接使用`Liuli`官方的配置`vim liuli/liuli_config/wechat.json`：
